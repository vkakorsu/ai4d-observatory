...
export async function generateStaticParams() {
  ...
  const res = await payload.find({ collection: 'use-cases', limit: 500, depth: 0, where: { _status: { equals: 'published' } } })
...
export default async function UseCasePage({ params }: Props) {
  const { slug } = await params
  const payload = await getPayloadClient()
  const [doc, settings] = await Promise.all([findBySlug<UseCase>(payload, 'use-cases', slug, 2), getSettings()])
  if (!doc) notFound()
  const show = Boolean(settings.showPrototypeNotices)
  const related = await relatedContent(payload, doc as unknown as Record<string, unknown>, { collection: 'use-cases', id: doc.id })
  ...
  return (
        ...
        <>
          <MetaList
            items={[
              ...
              Array.isArray(doc.people) && doc.people.length > 0 && { label: 'People', value: <PeopleLinks people={doc.people} /> },
    ...
    >
      ...
      <Facts
        items={[
          doc.problem && { label: 'Development problem', value: <span style={{ fontSize: 'var(--step-1)', fontFamily: 'var(--font-text)' }}>{doc.problem}</span> },
          doc.responsibleAiPractices && {
            label: 'Responsible AI practices',
            value: <span style={{ fontSize: 'var(--step-1)', fontFamily: 'var(--font-text)' }}>{doc.responsibleAiPractices}</span>,
          },
          doc.evidenceOfImpact && {
            label: 'Evidence of impact',
            value: <span style={{ fontSize: 'var(--step-1)', fontFamily: 'var(--font-text)' }}>{doc.evidenceOfImpact}</span>,
          },
        ]}
      />
