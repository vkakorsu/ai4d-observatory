...
import { Avatar, Breadcrumbs, PageHeader, PendingBadge, ProvenanceBadge, Section } from '@/components/ui'
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return buildMetadata({ title: doc?.title ?? 'About', description: doc?.summary, path: '/about', seo: doc?.seo })
}
...
export default async function AboutPage() {
  ...
  const [doc, settings, team, partners] = await Promise.all([
    ...
    getSettings(),
    payload.find({ collection: 'people', where: { and: [{ _status: { equals: 'published' } }, { affiliation: { equals: 'team' } }] }, sort: 'name', limit: 50, depth: 1, overrideAccess: false }),
    payload.find({ collection: 'organisations', where: { and: [{ _status: { equals: 'published' } }, { observatoryRole: { in: ['lead', 'partner', 'funder'] } }] }, sort: 'name', limit: 50, depth: 1, overrideAccess: false }),
  ])
  if (!doc) notFound()
    ...
    <div className="container">
      ...
      <div className="detail">
        <article>
          {show && (
            <p className="small">
              <ProvenanceBadge provenance={doc.provenance} show={show} /> Copy on this page is drawn from LIRNEasia’s public description of
              the Observatory and is a placeholder for the Client’s own text.
            </p>
          )}
          ...
          <div className="panel panel--tint">
            ...
            <p className="small" style={{ margin: 0 }}>
              {settings.contactEmail ? (
                ...
                <>
                  <PendingBadge>Pending client input</PendingBadge> Contact address to be set in Site settings.
                </>
              )}
            </p>
          </div>
      ...
      <Section id="partners" title="Led by LIRNEasia with partners" more={{ href: '/about/partners', label: 'Partners and funders' }}>
            ...
            <Link className="pcard" href={`/organisations/${o.slug}`} key={o.id}>
              ...
              <span>
                ...
                <span className="pcard__role" style={{ display: 'block' }}>
                  {o.observatoryRole === 'lead' ? 'Lead organisation' : o.observatoryRole === 'funder' ? 'Funder' : 'Consortium partner'}
                </span>
              </span>
            </Link>
          ))}
      ...
      <Section id="team" title="Team">
        {team.docs.length === 0 ? (
          <p className="muted">
            <PendingBadge>Pending client input</PendingBadge> Team profiles are added by editors as People with the affiliation “Observatory team”.
          </p>
        ) : (
          <div className="card-grid">
            {team.docs.map((p) => (
