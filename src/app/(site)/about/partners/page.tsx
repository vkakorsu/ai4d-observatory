...
export default async function PartnersPage() {
  ...
  const [settings, orgs] = await Promise.all([
    ...
    payload.find({
      ...
      where: {
        and: [{ _status: { equals: 'published' } }, { observatoryRole: { in: [...ROLE_ORDER] } }],
      },
      limit: 100,
      depth: 1,
      sort: 'name',
  ...
  return (
    <div className="container">
      <Breadcrumbs
        items={[{ href: '/about', label: 'About' }, { label: 'Partners and funders' }]}
      />
      <PageHeader
        kicker="About the Observatory"
        title="Partners and funders"
        lede={settings.programmeNote ?? undefined}
      />
      {show && (
        <p className="small">
          <PendingBadge>Pending client assets</PendingBadge> Logos and approved descriptions replace
          the text marks once the Client supplies brand assets and usage rules for each
          organisation.
        </p>
      )}
      {ROLE_ORDER.map((role) => {
