...
const TYPE: Record<string, string> = { course: 'Course', video: 'Video', toolkit: 'Toolkit', guide: 'Guide', framework: 'Framework', 'reading-list': 'Reading list' }
const LEVEL: Record<string, string> = { introductory: 'Introductory', intermediate: 'Intermediate', advanced: 'Advanced' }

export async function generateStaticParams() {
  const payload = await getPayloadClient()
  const res = await payload.find({ collection: 'learning-resources', limit: 500, depth: 0, where: { _status: { equals: 'published' } } })
  return res.docs.map((d) => ({ slug: d.slug ?? '' })).filter((p) => p.slug)
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params
  const doc = await findBySlug<LearningResource>(await getPayloadClient(), 'learning-resources', slug, 1)
  if (!doc) return {}
  return buildMetadata({ title: doc.title, description: doc.summary, path: `/learning/${slug}`, seo: doc.seo })
}

export default async function LearningResourcePage({ params }: Props) {
  const { slug } = await params
  const payload = await getPayloadClient()
  const [doc, settings] = await Promise.all([findBySlug<LearningResource>(payload, 'learning-resources', slug, 2), getSettings()])
  if (!doc) notFound()
  const show = Boolean(settings.showPrototypeNotices)
  const related = await relatedContent(payload, doc as unknown as Record<string, unknown>, { collection: 'learning-resources', id: doc.id })
  const org = doc.providerOrganisation && typeof doc.providerOrganisation === 'object' ? doc.providerOrganisation : null
        ...
        <>
          {doc.externalUrl && (
            <div className="download-box">
              ...
              <TrackLink className="btn btn--primary" href={doc.externalUrl} event="learning_open" data={{ resource: doc.title }} rel="noopener noreferrer">
          ...
          <MetaList
            items={[
              ...
              (doc.provider || org) && {
                label: 'Provider',
                value: org ? <Link href={`/organisations/${org.slug}`}>{org.name}</Link> : doc.provider,
              },
            ]}
          />
    ...
    >
      ...
      {embed && (
        <figure style={{ marginBottom: 'var(--s-5)' }}>
          <div style={{ aspectRatio: '16 / 9', border: 'var(--rule)', background: 'var(--paper-deep)' }}>
            <iframe
              src={embed}
              title={doc.title}
              width="100%"
          ...
          <figcaption className="tiny muted">
            Video is embedded from an external service. <a href={doc.videoEmbedUrl ?? '#'}>Open on the provider’s site</a>.
          </figcaption>
        </figure>
      )}
